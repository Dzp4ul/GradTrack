...
{
    global $sessionIds, $createdBlock, $groupPhotoCleanup, $groupMembershipCleanup, $temporaryFiles, $db;
...
try {
    ...
    $group = $db->query("SELECT r.id, r.created_by
                         FROM forum_chat_rooms r
                         JOIN forum_chat_members member ON member.room_id = r.id
                         WHERE r.is_group = 1
                         ...
                         ORDER BY r.id ASC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    if (!$group) throw new RuntimeException('No group fixture is available');
    ...
    $outsiderPhotoUpload = conversation_http_multipart($baseUrl . '/conversation-info.php', $outsiderSession, [
    ...
    $outsiderAvatar = conversation_http_redirect($avatarUrl, $outsiderSession);
