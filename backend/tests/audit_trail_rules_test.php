...
audit_trail_test_assert(gradtrack_audit_role_is_allowed('research_coordinator'), 'research coordinator activity is recorded');
audit_trail_test_assert(gradtrack_audit_role_is_allowed('admin'), 'admin activity is recorded');
audit_trail_test_assert(gradtrack_audit_role_is_allowed('alumni_president'), 'alumni president activity is recorded');
...
audit_trail_test_assert(gradtrack_audit_viewer_role_is_allowed('admin'), 'admin can open the audit trail');
audit_trail_test_assert(!gradtrack_audit_viewer_role_is_allowed('research_coordinator'), 'research coordinator cannot open the audit trail');
...
$coordinatorLogged = logAuditTrail(
    1,
    'admin@example.edu',
...
audit_trail_test_assert($coordinatorLogged === true, 'research coordinator activity is recorded');

$row = $db->query('SELECT * FROM audit_trail LIMIT 1')->fetch(PDO::FETCH_ASSOC);
audit_trail_test_assert(($row['user_name'] ?? '') === 'Research Coordinator #1', 'email fallback is not stored as actor name');
...
audit_trail_test_assert(($publicRow['role_label'] ?? '') === 'Research Coordinator', 'public audit rows include readable role labels');

logAuditTrail(2, 'Research Coordinator', 'research_coordinator', null, 'Generate', 'Reports', 'Generated graduate tracer report.');
logAuditTrail(3, 'Registrar Account', 'registrar', 'BSCS', 'Update', 'Graduate Records', 'Updated graduate record with ID 102.', 102);
logAuditTrail(4, 'Alumni President', 'alumni_president', 'BSCS', 'Approve', 'Job Posting', 'Approved job posting with record ID 25.', 25);
logAuditTrail(5, 'Dean Account', 'dean_cs', 'CCS', 'Login', 'Authentication', 'Logged in to the administrative portal.');

$allConditions = gradtrack_audit_build_conditions([], 'research_coordinator', 2);
...
$graduateFilter = gradtrack_audit_build_conditions(['user_role' => 'graduate'], 'research_coordinator', 2);
audit_trail_test_assert(audit_trail_count($db, $graduateFilter) === 0, 'graduate role filter returns no records');

$deanFilter = gradtrack_audit_build_conditions(['user_role' => 'Dean'], 'research_coordinator', 2);
audit_trail_test_assert(audit_trail_count($db, $deanFilter) === 1, 'dean role category filter expands to dean role variants');

$actionFilter = gradtrack_audit_build_conditions(['action' => 'Update'], 'research_coordinator', 2);
...
$searchFilter = gradtrack_audit_build_conditions(['search' => 'Research Coordinator'], 'research_coordinator', 2);
audit_trail_test_assert(audit_trail_count($db, $searchFilter) === 2, 'search matches every readable Research Coordinator role label');
...
$exportLikeFilter = gradtrack_audit_build_conditions(['module' => 'Job Posting', 'action' => 'Approve'], 'research_coordinator', 2);
