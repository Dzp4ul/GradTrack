...
function competencies_migration_normalize($value): string
{
...
function competencies_migration_has_other_option(array $options): bool
{
    foreach ($options as $option) {
        $normalized = competencies_migration_normalize($option);
...
try {
    ...
    $stmt = $db->query(
        "SELECT id, survey_id, question_text, question_type, options
         ...
         WHERE LOWER(question_text) LIKE '%what competencies were useful%'
    ...
    foreach ($questions as $question) {
        ...
        if (competencies_migration_has_other_option($options)) {
            $skipped[] = [
                'id' => (int) $question['id'],
                'survey_id' => (int) $question['survey_id'],
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Other option added to competency questions.',
