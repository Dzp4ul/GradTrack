...
function graduate_program_migration_normalize($value): string
{
...
function graduate_program_migration_has_other_option(array $options): bool
{
    foreach ($options as $option) {
        $normalized = graduate_program_migration_normalize($option);
...
try {
    ...
    $stmt = $db->query(
        "SELECT id, survey_id, question_text, question_type, options
         ...
         WHERE LOWER(TRIM(question_text)) = 'name of graduate program'
    ...
    foreach ($questions as $question) {
        ...
        if (graduate_program_migration_has_other_option($options)) {
            $skipped[] = [
                'id' => (int) $question['id'],
                'survey_id' => (int) $question['survey_id'],
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Other option added to Name of Graduate Program questions.',
