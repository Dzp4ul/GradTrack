...
function not_employed_reason_migration_normalize($value): string
{
...
function not_employed_reason_migration_has_other_option(array $options): bool
{
    foreach ($options as $option) {
        $normalized = not_employed_reason_migration_normalize($option);
...
try {
    ...
    $stmt = $db->query(
        "SELECT id, survey_id, question_text, question_type, options
         ...
         WHERE LOWER(question_text) LIKE '%reason%why you are not yet employed%'
    ...
    foreach ($questions as $question) {
        ...
        if (not_employed_reason_migration_has_other_option($options)) {
            $skipped[] = [
                'id' => (int) $question['id'],
                'survey_id' => (int) $question['survey_id'],
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Other option added to not-employed reason questions.',
